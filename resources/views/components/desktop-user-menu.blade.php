    ...
    <flux:menu class="min-w-56">
        <div class="flex items-center gap-2.5 px-2 py-2 text-start text-xs">
            ...
            <div class="grid flex-1 text-start leading-tight min-w-0">
                ...
                <span class="truncate text-slate-500 dark:text-zinc-400 text-[11px] font-medium">{{ auth()->user()->email }}</span>
            </div>
        </div>
        ...
        <flux:menu.radio.group>
            ...
            <flux:menu.item :href="route('teams.index')" icon="users" wire:navigate class="text-xs font-medium">
                {{ __('Teams') }}
            </flux:menu.item>
            <flux:menu.item :href="route('appearance.edit')" icon="swatch" wire:navigate class="text-xs font-medium">
                {{ __('Appearance') }}
            </flux:menu.item>
        </flux:menu.radio.group>
