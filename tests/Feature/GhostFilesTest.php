...
test('GhostFileService detects ghost candidates and generates valid stubs', function () {
    ...
    // Test dual-compatibility: XML/HTML attribute format generated by Obsidian plugin
    $pluginStub = "<!-- synkk:ghost path=\"assets/keynote.mp4\" size=\"52428800\" mime=\"video/mp4\" sha256=\"deadbeef12345678\" -->\n> [!NOTE] Ghost";
    expect($service->isGhostStub($pluginStub))->toBeTrue();

    $parsedPlugin = $service->parseGhostStub($pluginStub);
    expect($parsedPlugin)->not->toBeNull()
        ->and($parsedPlugin['path'])->toBe('assets/keynote.mp4')
        ->and($parsedPlugin['size'])->toBe(52428800)
        ->and($parsedPlugin['mime'])->toBe('video/mp4')
        ->and($parsedPlugin['sha256'])->toBe('deadbeef12345678');
});
